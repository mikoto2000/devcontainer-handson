...
次の公式サイトから VS Code をインストールしてください。

https://code.visualstudio.com/

インストール時に、次のチェックボックスを ON にするのを推奨（便利）。
...
スタートメニューから Windows Terminal を起動し、次コマンドを実行します。
...
再起動後、再度 Windows Terminal を起動し、次コマンドを実行します。
...
WARNING: この時点で、 BIOS の仮想化支援機能が無効になっている場合、エラーメッセージが表示されることがあります。その場合には、セクション「BIOS の仮想化支援機能の有効化」を参照して、BIOS の設定を変更し、もう一度 WSL のインストールをやり直してください。
...
BIOS 設定画面の操作方法は PC のメーカーやモデルによって異なります。次の手順で操作できない場合は、運営に問い合わせてください。
